...
long mod_hash(long key, long capacity) {
  return key & (capacity-1); // since the capacities are a power of 2
}

//murmur3 hash changed with starting position addition
...
Ht_mem_item *create_mem_item(typeArrayOopDesc *key, int startingPosition, typeArrayOopDesc *value) {
  // Creates a pointer to a new hash table item
  Ht_mem_item *item = (Ht_mem_item *) malloc(sizeof(Ht_mem_item));
...
Ht_cons_item *create_cons_item(typeArrayOopDesc *key, int compArrayLength, int arraysCreatedIndex) {
  // Creates a pointer to a new hash table item
...
Ht_dupl_item *create_dupl_item(typeArrayOopDesc *key, typeArrayOopDesc *value) {
  // Creates a pointer to a new duplicate table item
  Ht_dupl_item *item = (Ht_dupl_item *) malloc(sizeof(Ht_dupl_item));
...
HashMemTable *create_mem_table(int size) {
  // Creates a new HashTable
  HashMemTable *table = (HashMemTable *) malloc(sizeof(HashMemTable));
  table->size = size;
  table->count = 0;
  table->items = (Ht_mem_item **) calloc(table->size, sizeof(Ht_mem_item *));
...
HashDuplTable *create_dupl_table(int size) {
  // Creates a new HashTable
  HashDuplTable *table = (HashDuplTable *) malloc(sizeof(HashDuplTable));
  table->size = size;
  table->count = 0;
  table->items = (Ht_dupl_item **) calloc(table->size, sizeof(Ht_dupl_item *));
  for (int i = 0; i < table->size; i++)
    table->items[i] = NULL;
...
void free_dupl_item(Ht_dupl_item *item) {
  // Frees an item
  free(item->key);
  free(item->value);
  free(item);
...
//   // Frees the table
//   for (int i = 0; i < table->size; i++) {
//     Ht_mem_item *item = table->items[i];
//     if (item != NULL)
//       free_mem_item(item);
//   }
...
// void free_dupl_table(HashDuplTable *table) {
//   // Frees the table
//   for (int i = 0; i < table->size; i++) {
//     Ht_dupl_item *item = table->items[i];
//     if (item != NULL)
//       free_dupl_item(item);
//   }
...
void handle_mem_collision(HashMemTable *table, Ht_mem_item *item, int index) {
  for (int i = 1; i < MEM_COPY_CAPACITY; i++) {
    ...
    Ht_mem_item *current_item = table->items[modIndex];
    if (current_item == NULL) {
      ...
      table->items[modIndex]->startingPosition = item->startingPosition;
      return;
    }
  }
...
bool ht_insert_mem_map(HashMemTable *table, typeArrayOopDesc *key, jint startingPosition, typeArrayOopDesc *value) {
  // Create the item
...
bool ht_insert_consumer_map(HashConsTable *table, typeArrayOopDesc *key, jint compArrayLength, jint arraysCreatedIndex)
{
  // Create the item
  Ht_cons_item *item = create_cons_item(key, compArrayLength, arraysCreatedIndex);
...
void handle_dupl_collision(HashDuplTable *table, Ht_dupl_item *item, int index) {
  for (int i = 1; i < DUPL_CAPACITY; i++) {
    Ht_dupl_item *current_item = table->items[(index + i) % DUPL_CAPACITY];
    if (current_item == NULL) {
      table->items[(index + i) % DUPL_CAPACITY] = item;
      table->count++;
      return;
    } else if (item->key == current_item->key) {
      table->items[(index + i) % DUPL_CAPACITY]->value = item->value;
      return;
    }
  }
  printf("THE DUPL COLLISION COULD NOT BE RESOLVED");
}

bool ht_insert_dupl_map(HashDuplTable *table, typeArrayOopDesc *key, typeArrayOopDesc *value) {
  // Create the item
  Ht_dupl_item *item = create_dupl_item(key, value);

  // Compute the index
  int index = hash_function(key, -2);

  Ht_dupl_item *current_item = table->items[index];
  if (current_item == NULL) {
    // Key does not exist.
    if (table->count == table->size) {
      // Hash Table Full
      printf("INSERT ERROR: Duplicate Table is full\n");
      free_dupl_item(item);
      return false;
    }
    // Insert directly
...
void handle_mem_copies_collision(HashMemoryCopiesTable *table, Ht_mem_copies_item *item, int index) {
  for (int i = 1; i < MEM_COPY_CAPACITY; i++) {
    ...
    if (current_item == NULL) {
      ...
      table->items[modIndex]->valueAddress = item->valueAddress;
      table->items[modIndex]->startingPosition = item->startingPosition;
      return;
    }
  }
...
bool ht_insert_mem_copies_map(HashMemoryCopiesTable *table, long keyAddress, long valueAddress, long startingPosition) {
  ...
  // Compute the index
  long index = /*knuth_hash_longs(keyAddress);*/ mod_hash((long) keyAddress, (long) MEM_COPY_CAPACITY);
...
// typeArrayOopDesc *ht_search_mem_array(HashMemTable *table, typeArrayOopDesc *key) {
//   // Searches the key in the hashtable
//   // and returns NULL if it doesn't exist
//   int index = hash_function(key, -1);
//   Ht_mem_item *item = table->items[index];
...
//       for (int i = 1; i < MEM_COPY_CAPACITY; i++) {
//         Ht_mem_item *collisionItem = table->items[(index + i) & (MEM_COPY_CAPACITY-1)];
...
// typeArrayOopDesc *ht_search_dupl_array(HashDuplTable *table, typeArrayOopDesc *key) {
//   // Searches the key in the hashtable
//   // and returns NULL if it doesn't exist
//   int index = hash_function(key, -2);
//   Ht_dupl_item *item = table->items[index];
...
//       //CHECK FOR COLLISIONS
//       for (int i = 1; i < DUPL_CAPACITY; i++) {
//         Ht_dupl_item *collisionItem = table->items[(index + i) % DUPL_CAPACITY];
//         if (collisionItem != NULL) {
//           if (collisionItem->key == key) {
//             return collisionItem->value;
//           }
...
// jint ht_search_mem_starting_pos(HashMemTable *table, typeArrayOopDesc *key) {
//   // Searches the key in the hashtable
//   // and returns NULL if it doesn't exist
//   int index = hash_function(key, -1);
//   Ht_mem_item *item = table->items[index];
...
//       //CHECK FOR COLLISIONS
//       for (int i = 1; i < CONSUMER_CAPACITY; i++) {
//         Ht_cons_item *collisionItem = table->items[(index + i) & (CONSUMER_CAPACITY-1)];
//         if (collisionItem != NULL) {
//           if (collisionItem->key == key) {
//             return collisionItem->value;
//           }
      ...
      for (int i = 1; i < CONSUMER_CAPACITY; i++) {
        ...
        if (collisionItem != NULL) {
          if (collisionItem->key == key) {
            ...
            compItem.arraysCreatedIndex = collisionItem->arraysCreatedIndex;
            return;
          }
        } else {
          //The item is not here
...
// YES Object.hash_put() to add items in map, with 0 add to memcopy map and with 1 in consumer
JVM_ENTRY_NO_ENV(jboolean,
         JVMCIRuntime::object_hash_put(JNIEnv* env, typeArrayOopDesc* ar1, jint compArrayLength,
                                         jint arraysCreatedIndex))
   //inserting in consumer map
    //    pthread_mutex_lock(&mtxConsumer);
    jboolean retVal= ht_insert_consumer_map(htConsumer, ar1, compArrayLength, arraysCreatedIndex);
...
//   long keyAddress = (long) ar1;
long valueAddress = 1l;//ht_search_mem_copies_array(htMemoryCopies, keyAddress);
...
// Object.hash_dupl_array_get() fast path, caller does slow path
JRT_LEAF(void , JVMCIRuntime::object_hash_dupl_array_get(JavaThread* thread, typeArrayOopDesc* ar1))
//   typeArrayOopDesc* valueArray = ht_search_dupl_array(htDuplicate, ar1);
...
// YES Object.hash_consumer_get() fast path, caller does slow path
...
//  thread-> set_vm_result(keepReturnItem.arSize);
JRT_END
...
//   thread-> set_vm_result(keepReturnItem.lengths);
...
int x=0;
// YES Object.my_debug_print() fast path, caller does slow path
